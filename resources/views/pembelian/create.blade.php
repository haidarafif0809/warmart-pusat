		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<span style="display: none;">
					...
					@endif
					{!! Form::hidden('status_pembelian', 'Kredit', ['class'=>'form-control','autocomplete'=>'off', 'id'=>'status_pembelian']) !!}	
							...
							<div class="col-md-8">
								...
								<div class="col-md-4"><br>
									<div class="{{ $errors->has('id_produk') ? ' has-error' : '' }}">
										{!! Form::select('id_produk', []+App\Barang::where('status_aktif',1)->where('id_warung',Auth::user()->id_warung)->select([DB::raw('CONCAT(kode_barang, " - ", nama_barang) AS data_produk'),'id'])->pluck('data_produk','id')->all(), null, ['class'=>'', 'placeholder' => '-- PILIH PRODUK --', 'id'=>'pilih_produk','autofocus' => 'true']) !!}
		...
		$select.on('change', function(){
			...
			if (produk == "") {
				...
				swal({
					...
					type: 'question',
					html:'Berapa Jumlah Yang akan di masukan?',	
					animation: false,
...
<script type="text/javascript">
	$(document).on('click', '.edit-tax', function () {
		...
		swal({
			...
			'<select id="ppn_swal" name="ppn_swal" class="js-selectize-reguler">'+
			'<option value"Include>Include</option>'+
			'<option value"Exclude>Exclude</option>'+
			'</select>' +
			'<input type="text" id="tax_swal" class="tax_swal" name="tax_swal">',
			animation: false,
			showCloseButton: true,
			showCancelButton: true,
			focusConfirm: true,
			...
			closeOnConfirm: true,
			cancelButtonAriaLabel: 'Thumbs down',
			inputClass : 'js-selectize-reguler',
			inputAttributes: {
				'name': 'edit_tax_produk',
			}, 
			preConfirm: function () {
