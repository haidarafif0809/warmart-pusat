...
class TransaksiHutang extends Model
{
 ...
 public function scopeGetDataPembelianHutang($data_supplier_hutang,$id_suplier) {
  $data_supplier_hutang = TransaksiHutang::select(['pembelians.id', 'pembelians.no_faktur','pembelians.created_at', 'pembelians.suplier_id', 'pembelians.total', 'pembelians.tanggal_jt_tempo',DB::raw('IFNULL(SUM(transaksi_hutangs.jumlah_masuk),0) - IFNULL(SUM(transaksi_hutangs.jumlah_keluar),0) AS sisa_hutang'), 'transaksi_hutangs.id_transaksi'])
  ->leftJoin('pembelians', 'transaksi_hutangs.id_transaksi', '=', 'pembelians.id')
  ->leftJoin('supliers', 'supliers.id', '=', 'pembelians.suplier_id')
  ->where('pembelians.status_pembelian', 'Hutang')
...
public function scopeHutangTerbayar($query, $no_faktur_retur, $id_transaksi) {
  $query->select(DB::raw('IFNULL(SUM(jumlah_keluar),0) AS jumlah_keluar'))
  ->where('no_faktur', $no_faktur_retur)->where('id_transaksi', $id_transaksi);
