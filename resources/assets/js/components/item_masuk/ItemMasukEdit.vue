...
<template>
	...
	<div class="row">
      ...
      <div class="modal" id="modalJumlahProduk" role="dialog" tabindex="-1"  aria-labelledby="myModalLabel" aria-hidden="true" >
        <div class="modal-dialog modal-medium">
          <div class="modal-content">
            ...
            <form class="form-horizontal" v-on:submit.prevent="submitProdukItemMasuk(inputTbsItemMasuk.jumlah_produk)"> 
              <div class="modal-body text-center">
                <h3><b>{{inputTbsItemMasuk.nama_produk}}</b> </h3>
                <input class="form-control" type="number" v-model="inputTbsItemMasuk.jumlah_produk" placeholder="Isi Jumlah Produk" name="jumlah_produk" id="jumlah_produk" ref="jumlah_produk" autocomplete="off" step="0.01">
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-simple" v-on:click="closeModalJumlahProduk()">Close</button>
                <button type="button" class="btn btn-info btn-lg" v-on:click="submitProdukItemMasuk(inputTbsItemMasuk.jumlah_produk)">Tambah</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div class="modal" id="modalSelesaiItemMasuk" role="dialog" tabindex="-1"  aria-labelledby="myModalLabel" aria-hidden="true" >
        <div class="modal-dialog modal-medium">
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close"  v-on:click="closeModalSelesaiItemMasuk()"> &times;</button> 
            </div>
            <form class="form-horizontal" v-on:submit.prevent="prosesSelesaiItemMasuk"> 
              <div class="modal-body text-center">
                <h4><b>Anda Yakin Ingin Menyelesaikan Transaksi Ini ?</b> </h4>
                <input class="form-control" type="text" v-model="inputTbsItemMasuk.keterangan" placeholder="Silakan isi keterangan jika diperlukan" name="keterangan" id="keterangan" ref="keterangan" autocomplete="off">
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-simple" v-on:click="closeModalSelesaiItemMasuk()">Close</button>
                <button type="button" class="btn btn-info btn-lg" v-on:click="prosesSelesaiItemMasuk">Selesai</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div class="card">
       ...
       <div class="card-content">
         <h4 class="card-title">Edit Item Masuk {{ inputTbsItemMasuk.no_faktur }}</h4>
         <div class="row">
     ...
     <tbody v-if="tbs_item_masuk.length"  class="data-ada">
      <tr v-for="tbs_item_masuk, index in tbs_item_masuk" >
       ...
       <td>
        <a v-bind:href="'#edit-item-masuk/'+tbs_item_masuk.id_item_masuk" v-bind:id="'edit-' + tbs_item_masuk.id_edit_tbs_item_masuk" v-on:click="editEntry(tbs_item_masuk.id_edit_tbs_item_masuk, index,tbs_item_masuk.nama_produk)">{{ tbs_item_masuk.jumlah_produk | pemisahTitik }}
        </a>
      </td>
      <td> 
        <a v-bind:href="'#edit-item-masuk/'+tbs_item_masuk.id_item_masuk" class="btn btn-xs btn-danger" v-bind:id="'delete-' + tbs_item_masuk.id_edit_tbs_item_masuk" v-on:click="deleteEntry(tbs_item_masuk.id_edit_tbs_item_masuk, index,tbs_item_masuk.nama_produk)">Delete</a>
      </td>
    </tr>
  </tbody>					
...
export default {
  ...
  watch: {
      ...
      getResults(page) {
       ...
       axios.get(app.url+'/view-edit-tbs-item-masuk/'+id+'?page='+page)
       .then(function (resp) {
     ...
     axios.get(app.url+'/pencarian-edit-tbs-item-masuk/'+id+'?search='+app.pencarian+'&page='+page)
     .then(function (resp) {
   ...
   getFakturItemMasuk(){
    ...
    axios.get(app.url+'/ambil-faktur-item-masuk/'+id).then(function (resp) {
      ...
      app.inputTbsItemMasuk.no_faktur = resp.data.no_faktur; 
      app.inputTbsItemMasuk.keterangan = resp.data.keterangan;        
 ...
 inputJumlahProduk(nama_produk){
  var app = this
  app.inputTbsItemMasuk.nama_produk = nama_produk
...
submitProdukItemMasuk(value){
  ...
  if (value == 0) {
    ...
    app.loading = true;
    axios.post(app.url+'/proses-tambah-edit-tbs-item-masuk', newinputTbsItemMasuk)
    .then(function (resp) {
...
editJumlahProdukItemMasuk(value,id,nama_produk){
  if (value == 0) {
    ...
    var newinputTbsItemMasuk = app.inputTbsItemMasuk;
    app.loading = true;
    axios.post(app.url+'/edit-jumlah-edit-item-masuk', newinputTbsItemMasuk)
    .then(function (resp) {
...
prosesDelete(id,nama_produk){
  ...
  var app = this;
  app.loading = true;
  axios.post(app.url+'/proses-hapus-edit-tbs-item-masuk/'+id)
  .then(function (resp) {
    app.getResults();
...
batalItemMasuk(){
  ...
  .then((willDelete) => {
    if (willDelete) {
      ...
      app.loading = true;
      var newinputTbsItemMasuk = app.inputTbsItemMasuk;
      axios.post(app.url+'/proses-hapus-semua-edit-tbs-item-masuk',newinputTbsItemMasuk)
      .then(function (resp) {
...
prosesSelesaiItemMasuk(value){
  ...
  axios.post(app.url+'/proses-edit-item-masuk/'+id,newinputTbsItemMasuk)
  .then(function (resp) {
...
closeModalSelesaiItemMasuk(){    
  $("#modalSelesaiItemMasuk").hide(); 
}
