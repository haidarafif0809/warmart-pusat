	...
	export default {
		...
		data: function () {
			return {
				...
				url: window.location.origin + (window.location.pathname).replace("dashboard", "laporan-penjualan-harian-online"),
			}
		},
		mounted () {
			var app = this;
			...
			barChart(){
				...
				.then(function (resp) {
					app.renderChart(
					{
						...
						datasets: [{
							label: 'LAPORAN HARIAN PENJUALAN ONLINE '+ app.tanggal(filter.dari_tanggal) +" - "+ app.tanggal(filter.sampai_tanggal),
