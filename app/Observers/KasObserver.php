...
class KasObserver
{
    ...
    {       
        $data_kas = Kas::select('default_kas')->where('id', $Kas->id)
        ->where('warung_id', $Kas->warung_id)->first();
        $data_transaksi_kas = TransaksiKas::select('kas')->where('kas',$Kas->id)
        ->where('warung_id', $Kas->warung_id)->count();
...
BankWarung::where('id', $Kas->id_bank)->where('warung_id', $Kas->warung_id)->delete();
