...
class KasKeluarObserver
{
	...
	public function updating(KasKeluar $KasKeluar){
		...
		TransaksiKas::where('no_faktur' , $KasKeluar->no_faktur)->where('warung_id', Auth::user()->id_warung)->update(['jumlah_keluar' => $KasKeluar->jumlah,'kas' => $KasKeluar->kas] );
    }

	public function deleting(KasKeluar $KasKeluar){

		TransaksiKas::where('no_faktur',$KasKeluar->no_faktur)->where('warung_id', Auth::user()->id_warung)->delete();
    }
}