                            ...
                            <div class="card card-produk" style="margin-bottom: 1px; margin-top: 1px;">
                                <div class="form-group" style="margin-right: 10px; margin-left: 10px;">
                                    <selectize-component v-model="inputTbsPembayaranPiutang.penjualan_piutang" :settings="placeholder_piutang" id="penjualan_piutang" ref='penjualan_piutang'> 
                                        <option v-for="piutangs, index in penjualan_piutang" v-bind:value="piutangs.id">{{piutangs.id}} || {{ piutangs.nama_pelanggan }}</option>
                                    </selectize-component>
...
<script>
    export default {
        ...
        methods: {
            ...
            dataKas() {
                var app = this;
                axios.get(app.url_piutang+'/pilih-kas').then((resp) => {
                    app.kas = resp.data;   
            ...
            dataPiutang() {
                var app = this;
                axios.get(app.url_piutang+'/pilih-penjualan-piutang').then((resp) => {
                    app.penjualan_piutang = resp.data;
                    console.log('then dataPiutang: ', resp)
                })
                .catch((resp) => {
